...
inode_manager::inode_manager()
{
  ...
  uint32_t root_dir = alloc_inode(extent_protocol::T_DIR,0,0,0);
  if (root_dir != 1) {
 ...
 * Return its inum. */
uint32_t
inode_manager::alloc_inode(uint32_t type,unsigned long mode,unsigned short uid,unsigned short gid)
{
  /* 
   * your lab1 code goes here.
...
inode_manager::setattr(uint32_t inum, extent_protocol::attr &a)
{
  ...
  ino->gid=a.gid;
  put_inode(inum,ino);
  printf("uuuuuuuuuuuuuuuuuuuu%o",a.mode);
